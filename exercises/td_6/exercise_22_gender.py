...
def analyze_gender():
    ...
    tips.tail()
    tip = tips['tip'].values
    idx = pd.Categorical(tips['sex'], categories=['Male', 'Female']).codes
    ...
    az.plot_posterior(means_diff, ref_val=0, ax=ax, hdi_prob=0.8)
    ax.set_title('Difference between male and Female')
...
if __name__ == "__main__":
    ...
    # The data indicates that the probability that males get, on average, bigger tips
    # than females is more than 90%. With at least 80% probability we can say that the gender
    # is a factor on the amount of tips received.
