...
if __name__ == "__main__":
    ...
    centered_weight = filtered_data["weight"] - np.average(filtered_data["weight"])
    ...
    with pm.Model() as over_18_heights:
        ...
        mu = pm.Deterministic("mu", alpha + beta * centered_weight)
    ...
    # Plot the 99% HDI
    ax = az.plot_hdi(centered_weight, trace_over_18_heights['mu'], hdi_prob=0.99, color='k')
    ax.set_xlabel('weight')
    ax.set_ylabel('height')
    # Plot the regression line
    ax.plot(centered_weight, alpha_m + beta_m * centered_weight, c='k', label=f'y = {alpha_m:.2f} + {beta_m:.2f} * x')
    ax.legend()
    plt.show()
    ...
    ax = az.plot_hdi(centered_weight, ppc['height_pred'], hdi_prob=0.75, color='gray')
    # and the 95% HDI of the predicted data
    az.plot_hdi(centered_weight, ppc['height_pred'], hdi_prob=0.95, color='gray', ax=ax)

    # This is just to plot the original data
    plt.plot(centered_weight, height, 'b.')
    # This is to plot the regression line
    plt.plot(centered_weight, alpha_m + beta_m * centered_weight, c='k',
    ...
    # Note that because we are centering the data, the value we obtain for alpha (154.48)
